 ...
 */
function rowToProfileData(headers: string[], values: string[]): Partial<CSVProfileRow> {
 ...
 */
function validateProfileData(profileData: Partial<CSVProfileRow>, rowIndex: number): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (!profileData.email) {
    ...
    if (!emailRegex.test(profileData.email)) {
      errors.push(`Row ${rowIndex}: Invalid email format '${profileData.email}'`);
    }
  }
 ...
 */
function normalizeProfileData(profileData: Partial<CSVProfileRow>): CSVProfileRow {
  return {
 ...
 */
export async function importProfilesFromCSV(csvContent: string): Promise<ImportResult> {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        if (!existingMember) {
          result.errors.push(`Row ${rowIndex}: Member with email '${profileData.email}' not found`);
          result.skipped++;
          result.summary.invalidRows++;
          continue;
        }
    ...
    throw new Error(`Profile import failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}